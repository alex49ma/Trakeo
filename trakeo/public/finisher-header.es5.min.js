...
!function (t) {
    // ---------------------------------------------------------
    // CONFIGURACIÓN: Aquí defines tus 10 posiciones (0.0 a 1.0)
    // x: 0 es izquierda, 1 es derecha
    // y: 0 es arriba, 1 es abajo
    // ---------------------------------------------------------
    var posicionesFijas = [
        { x: 0.0845, y: 0.1523 }, // Superior izquierda (esquina)
        { x: 0.2341, y: 0.6784 }, // Inferior izquierda media
        { x: 0.4129, y: 0.0932 }, // Superior centro-izquierda
        { x: 0.5678, y: 0.5210 }, // Centro puro
        { x: 0.8912, y: 0.1145 }, // Superior derecha (esquina)
        { x: 0.1256, y: 0.9234 }, // Inferior izquierda profunda
        { x: 0.4823, y: 0.8456 }, // Inferior centro
        { x: 0.7345, y: 0.3891 }, // Centro derecha medio
        { x: 0.9567, y: 0.7231 }, // Derecha baja
        { x: 0.3104, y: 0.3345 }  // Centro superior izquierdo
    ];
    ...
    var e = function () {
        function t(i, index, h) { // 'index' recibe el número de partícula
            _ccc(this, t), this.o = h, this.r = s(i), this.d = this.grd(), this.h = this.grs(), this.s = Math.abs(this.gnfr(this.o.size)),
                this.srpgq(index), // Pasamos el índice a la función de posición
                this.vx = this.gnfr(this.o.speed.x) * this.grd(), this.vy = this.gnfr(this.o.speed.y) * this.grd()
        }
        return _cc(t, [{
            key: "srpgq",
            value: function (index) {
                // MODIFICADO: Usar posiciones fijas en lugar de aleatorias
                // Usamos el operador % para que si hay más de 10 partículas, se repitan las posiciones
                var pos = posicionesFijas[index % posicionesFijas.length];

                // Calculamos la posición en píxeles basándonos en el ancho/alto actual
        ...
        return _cc(h, [{
            ...
            value: function () {
                ...
                for (var s = 0; s < this.o.ac; s++) {
                    // MODIFICADO: Pasamos 's' directamente como segundo argumento para controlar la posición
